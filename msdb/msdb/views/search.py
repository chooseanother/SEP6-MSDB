...
def search_movie(queries : list):
    movies_result = Movie.objects.filter(reduce(operator.and_, (Q(title__icontains=x) for x in queries)))[:10]
    # if no movies are found, do the search with or operator instead
    if movies_result.count() == 0:
        movies_result = Movie.objects.filter(reduce(operator.or_, (Q(title__icontains=x) for x in queries)))[:10]

    return movies_result


def search_users(queries: list):
    users = User.objects.filter(reduce(operator.and_, (Q(name__icontains=x) for x in queries))).annotate(num_reviews=Count('reviews')).order_by('num_reviews')[:10]
    # if no users are found, do the search with or operator instead
    if users.count() == 0:
        users = User.objects.filter(is_staff=False).filter(reduce(operator.or_, (Q(name__icontains=x) for x in queries)))[:10]
...
def search_directors(queries: list) -> list:
    directors = []
    directors_qset = Movie.objects.values('director').filter(director__isnull=False).filter(reduce(operator.and_, (Q(director__icontains=x) for x in queries))).values_list('director', flat=True)
    # if no directors are found, do the search with or operator instead
    if len(directors_qset) == 0:
        directors_qset = Movie.objects.values('director').filter(director__isnull=False).filter(reduce(operator.or_, (Q(director__icontains=x) for x in queries))).values_list('director', flat=True)
...
def search_actors(queries: list) -> list:
    actors = []
    actors_qset = Movie.objects.values('actors').filter(actors__isnull=False).filter(reduce(operator.and_, (Q(actors__icontains=x) for x in queries))).values_list('actors', flat=True)
    # if no actors are found, do the search with or operator instead
    if len(actors_qset) == 0:
        actors_qset = Movie.objects.values('actors').filter(actors__isnull=False).filter(reduce(operator.or_, (Q(actors__icontains=x) for x in queries))).values_list('actors', flat=True)
